defmodule RedisGraph.QueryResultTest do
  ...
  setup context do
    on_exit(fn ->
      conn = (context[:conn])
      graph = (context[:graph])
      RedisGraph.delete(conn, graph.name) end
    )
    :ok
  end
  ...
  describe "Hard-coded queries:" do
    ...
    test "test that number of elements in header is the same as requested", %{conn: conn, graph: graph} = _context do
      query = "CREATE (n:Person {age: 10, hobbies: ['sports', 'cooking'], name: 'John'})-[r:HAS]->(m:Dog {age: 2, name: 'Charlie'}) RETURN n, r, m, m.age"
      {:ok, query_result} = RedisGraph.query(conn, graph.name, query)
    ...
    test "test that number of elements in result_set is the same as requested", %{conn: conn, graph: graph} = _context do
      query = "CREATE (n:Person {age: 10, hobbies: ['sports', 'cooking'], name: 'John'})-[r:HAS]->(m:Dog {age: 2, name: 'Charlie'}) RETURN n, r, m, m.age"
      {:ok, query_result} = RedisGraph.query(conn, graph.name, query)
    ...
    test "test that returned values are of correct data type", %{conn: conn, graph: graph} = _context do
      query = "CREATE (n:Person {age: 10, hobbies: ['sports', 'cooking'], name: 'John', stuff: null, money: 22.2, works: false})-[r:HAS {duration: 1}]->(m:Dog {age: 2, name: 'Charlie'}) RETURN n, r, m, n.age, n.hobbies, n.name, n.stuff, n.money, n.works, r.duration, m.name"
      {:ok, query_result} = RedisGraph.query(conn, graph.name, query)
      result_set = Map.get(query_result, :result_set, []) |> List.first([])
      # IO.puts("result_set")
      # IO.inspect(result_set)
      [n, r, m, n_age, n_hobbies, n_name, n_stuff, n_money, n_works, r_duration, m_name] = result_set
    ...
    test "test that returned values have correct values", %{conn: conn, graph: graph} = _context do
      query = "CREATE (n:Person {age: 10, hobbies: ['sports', 'cooking'], name: 'John', stuff: null, money: 22.2, works: false})-[r:HAS {duration: 1}]->(m:Dog {age: 2, name: 'Charlie'}) RETURN n, r, m, n.age, n.hobbies, n.name, n.stuff, n.money, n.works, r.duration, m.name"
      {:ok, query_result} = RedisGraph.query(conn, graph.name, query)
      result_set = Map.get(query_result, :result_set, []) |> List.first([])
      # IO.puts("result_set")
      # IO.inspect(result_set)
      [n, r, m, n_age, n_hobbies, n_name, n_stuff, n_money, n_works, r_duration, m_name] = result_set
      [n_correct, r_correct, m_correct, n_age_correct, n_hobbies_correct, n_name_correct, n_stuff_correct, n_money_correct, n_works_correct, r_duration_correct, m_name_correct] = [
        %RedisGraph.Node{id: 0, alias: :n, labels: ["Person"], properties: %{ age: 10, hobbies: ["sports", "cooking"], money: 22.2, name: "John", works: false}},
        %RedisGraph.Relationship{id: 0, alias: :r, src_node: 0, dest_node: 1, type: "HAS", properties: %{duration: 1}},
        %RedisGraph.Node{id: 1, alias: :m, labels: ["Dog"], properties: %{age: 2, name: "Charlie"}},
        10,
        ["sports", "cooking"],
        "John",
        nil,
    ...
    test "test that query returns two sets of entities that match the condition", %{conn: conn, graph: graph} = _context do
      query = "CREATE (n:Person {age: 11, name: 'John'})-[r:HAS {duration: 1}]->(n:Dog {age: 2, name: 'Charlie'}), (n:Person {age: 22, name: 'Mike'})-[t:HAS {duration: 1}]->(m) " <>
  ...
  describe "built queries:" do
    test "create nodes with relatioships and test for multiple things", %{conn: conn, graph: graph} = _context do
      ...
      {:ok, query} = Query.new
                      |> Query.create
                      |> Query.node(:n, ["Person"], %{ age: 10, hobbies: ["sports", "cooking"], name: "John", stuff: nil,  money: 22.2, works: false})
  ...
  test "create nodes with relatioships, then delete and lastly test for multiple things", %{conn: conn, graph: graph} = _context do
    # same as "CREATE (n:Person {age: 10, hobbies: ['sports', 'cooking'], money: 22.2, name: 'John', stuff: null, works: false})-[r:HAS {duration: 1}]->(m:Dog {age: 2, name: 'Charlie'}) RETURN n, r, m, n.age, n.hobbies, n.name, n.stuff, n.money, n.works, r.duration, m.name"
    {:ok, create_query} = Query.new
                          |> Query.create
                          |> Query.node(:n, ["Person"], %{ age: 10, hobbies: ["sports", "cooking"], name: "John", stuff: nil,  money: 22.2, works: false})
                          |> Query.relationship_from_to(:r, "HAS", %{duration: 1})
                          |> Query.node(:m, ["Dog"], %{age: 2, name: "Charlie"})
